...
availableRobots rc bp rs = filter (\robot -> (==) 0 $ length $ filter (> 0) (Map.elems $ subtractResources rc (bp Map.! robot))) rs
...
neighbors State { robots = rs, resources = rc } bp = 
    let newResources = addResources rc $ mineralsToRC $ map mineral rs
        newRobotPossibilities = availableRobots newResources bp [Robot Ore, Robot Clay, Robot Obsidian, Robot Geode] in
            ...
            map (\robot -> State { robots = robot:rs, resources = subtractResources newResources (bp Map.! robot) }) newRobotPossibilities -- ugh, need to subtract resources from newResources for every robot...
