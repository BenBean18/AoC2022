...
parseOperation matches modsMultiplied a =
...
monkFunc operation test true false id monkeys =
    let currentItems = items (monkeys !! id) in
        if length currentItems == 0 then monkeys
            ...
            let thisMonkey = (monkeys !! id)
                thisItem = operation (head currentItems)
                monkeyToThrowTo = if test thisItem then true else false
                destMonkey = (monkeys !! monkeyToThrowTo)
                destMonkeyItems = items destMonkey
                newDestMonkeyItems = insert1D 0 thisItem destMonkeyItems
                newSelfItems = tail currentItems
                newMonkeys = (set1D id thisMonkey { items = newSelfItems, interactedCount = interactedCount (thisMonkey) + 1 } (set1D monkeyToThrowTo destMonkey { items = newDestMonkeyItems } monkeys)) in
...
parseMonkey !monkey !modsMultiplied =
    let id = read (last (head (monkey =~ "Monkey (\\d+):" :: [[String]]))) :: Int
        ...
        operation = parseOperation (operationRaw =~ "old ([*+]) (\\d+|(?:old))") modsMultiplied
        testRaw = monkey =~ "Test: .+" :: String
        divisor = getDivisor monkey
        test = (\i -> i `mod` divisor == 0)
        true = read (last (head (monkey =~ "If true: throw to monkey (\\d+)" :: [[String]]))) :: Int
        false = read (last (head (monkey =~ "If false: throw to monkey (\\d+)" :: [[String]]))) :: Int
        thisMonkFunc = monkFunc operation test true false id in
            Monkey { items = startingItems, divisor = divisor, fn = thisMonkFunc, interactedCount = 0 }

parseMonkeys :: [String] -> [Monkey]
parseMonkeys strs =
    let divviedStrs = (divvy 6 7 strs)
        concattedStrs = map (concat . (\y -> map (\x -> x ++ "\n") y)) divviedStrs
        firstMonkeys = map (\x -> parseMonkey x 0) concattedStrs
        modsMultiplied = multiplyMods firstMonkeys in
            map (\x -> parseMonkey x modsMultiplied) concattedStrs
